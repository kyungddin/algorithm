...
### 메모
1. 출력초과 문제: Vertex, edge, weight 값을 모두 고려해서 최대값/최소값을 파악하고 이를 기반으로 자료형을 선정하자.
2. 자료형을 선정했다면 큰 값을 담는 tmp나 거리 배열에 대해 그러한 자료형을 적용해주자.
3. tuple 자료형을 처음 써본 문제
4. 최대값을 define하지말고 편하게 limits.h를 활용하자.
