...
### 메모
1. 이분 그래프에 대해 배울 수 있었다
2. DFS, BFS에서 어떤 상태에 대한 업데이트는 반드시 알고리즘을 따라 진행되어야 한다. 따라서 전역변수로 업데이트 하는 방식보다는 매개변수를 사용하거나, 그 함수의 변수를 기준으로 진행하는 것이 좋다
